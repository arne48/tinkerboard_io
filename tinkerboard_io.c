...
void _write_mem_b(volatile uint32_t* mem_addr, uint32_t value){
...
uint32_t _read_mem_b(volatile uint32_t* mem_addr){
	uint32_t ret = 0;
	__sync_synchronize();
	ret = *mem_addr;
	__sync_synchronize();
...
printf("%d", ALIGN_TO_UINT32T(16));*/
	if(tinkerboard_init() == 1){
		...
		_read_mem_b(_rk3288_gpio_grf_base + 0x0054 / 4);
		_write_mem_b(_rk3288_gpio_5_base + 0x0001, (uint32_t) 1<<19);
		_read_mem_b(_rk3288_gpio_5_base + 0x0001);
		printf("GPIO init\n");

		_write_mem_b(_rk3288_gpio_5_base, (uint32_t) 1<<19);
